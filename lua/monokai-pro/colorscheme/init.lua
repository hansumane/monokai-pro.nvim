...
M.get = function(filter)
  local filters = { "light", "classic", "machine", "octagon", "pro", "ristretto", "spectrum", "molokai" }

  if not vim.tbl_contains(filters, filter) then
    local msg = 'Invalid filter, expected  "light", "classic", "machine", "octagon", "pro", "ristretto", "spectrum" or "molokai"'
    local level = "info"
    filter = "pro"
    Util.log(msg, level)
  end
